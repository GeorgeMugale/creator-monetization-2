...
const CreatorDashboard = () => {
  ...
  // Logic to determine which "mode" the page is in
  ...
  useEffect(() => {
    ...
    fetchData();
  }, [page, isTransactionsView]); // Refetch if page changes or user switches tabs
  ...
  const statCards = [
    { label: "Balance", val: data?.balance, icon: DollarSign, color: "text-zed-green", bg: "bg-green-50" },
    { label: "Incoming", val: data?.totalIncoming, icon: TrendingUp, color: "text-blue-600", bg: "bg-blue-50" },
    { label: "Outgoing", val: data?.totalOutgoing, icon: ArrowDownLeft, color: "text-orange-600", bg: "bg-orange-50" },
  ];
      ...
      <div className="mb-10 flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <div className="flex items-center gap-2 mt-1">
            ...
            {data?.kycVerified && <ShieldCheck size={14} className="text-zed-green" />}
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-10">
          {statCards.map((stat, i) => (
            <div key={i} className="bg-white p-6 rounded-[2rem] border border-gray-100 shadow-sm">
              <div className={`${stat.bg} ${stat.color} w-10 h-10 flex items-center justify-center rounded-xl mb-4`}>
                <stat.icon size={20} />
              </div>
      ...
      <div className="bg-white rounded-[2rem] border border-gray-100 shadow-sm overflow-hidden">
          ...
          <table className="w-full text-left">
            <thead>
              <tr className="bg-gray-50/50">
                ...
                <th className="px-8 py-4 text-[10px] font-black text-gray-400 uppercase">Amount</th>
              </tr>
            </thead>
        ...
        {isTransactionsView && data?.pagination?.pages > 1 && (
          <div className="p-6 bg-gray-50 border-t border-gray-100 flex justify-between items-center">
             ...
             <span className="text-xs font-bold">Page {page} of {data.pagination.pages}</span>
             <button 
